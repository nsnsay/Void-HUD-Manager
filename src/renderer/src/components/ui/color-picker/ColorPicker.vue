  ...
  <div class="relative">
    ...
    <Transition name="fade">
      <div v-if="isOpen"
        ...
        style="left: -350%;">
        <div class="space-y-4">
          <div ref="saturationRef" class="relative w-full aspect-4/2 rounded border cursor-crosshair" :style="{
            ...
            <div class="absolute w-4 h-4 border-2 border-white rounded-full pointer-events-none" :style="{
              left: `${colorHsv.s}%`,
              top: `${100 - colorHsv.v}%`,
          ...
          <div ref="hueRef" class="relative w-full h-4 rounded cursor-pointer" :style="{
            background:
              'linear-gradient(to right, #f00 0%, #ff0 17%, #0f0 33%, #0ff 50%, #00f 67%, #f0f 83%, #f00 100%)',
          }" @mousedown="startHueDrag" @touchstart="startHueDrag" @click="handleHueClick">
            <div class="absolute w-4 h-4 border-2 border-foreground rounded-full pointer-events-none bg-white" :style="{
          ...
          <div class="flex items-center gap-2">
            <div class="relative">
              ...
              <div v-if="isDropdownOpen"
                ...
                <div class="p-1">
                  <button v-for="option in colorOptions" :key="option.value" type="button"
            ...
            <div class="flex grow">
              <input v-if="colorType === 'hex'" :value="hexValue" type="text"
                ...
                placeholder="#000000" @input="updateFromHex(($event.target as HTMLInputElement).value)" />

              <ObjectColorInput v-else :value="colorType === 'hsl' || colorType === 'hsla' ? hslValue : rgbValue"
                :label="colorType" @value-change="updateFromObject" />
            </div>
          </div>

          <div v-if="colorType === 'rgba' || colorType === 'hsla'" class="space-y-2">
            <div ref="alphaRef" class="relative w-full h-4 rounded cursor-pointer" :style="{
          ...
          <div v-if="swatches.length > 0 || !hideDefaultSwatches">
            ...
            <div v-if="!hideDefaultSwatches" class="flex flex-wrap justify-start gap-2 pt-2">
              <button v-for="color in sortedSwatches" :key="`${color}-swatch`" type="button"
                ...
                @click="setColorFromHex(color)" @keydown.enter="setColorFromHex(color)" />
            </div>
          </div>
